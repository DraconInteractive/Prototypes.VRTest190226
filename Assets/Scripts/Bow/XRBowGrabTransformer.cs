...
namespace UnityEngine.XR.Interaction.Toolkit.Transformers
{
    ...
    [BurstCompile]
#endif
    public class XRBowGrabTransformer : XRBaseGrabTransformer
    {
        /// <summary>
        ...
        /// </summary>
        /// <seealso cref="UnityEngine.XR.Interaction.Toolkit.Transformers.XRGeneralGrabTransformer.ManipulationAxes"/>
        ...
        /// </summary>
        /// <seealso cref="UnityEngine.XR.Interaction.Toolkit.Transformers.XRGeneralGrabTransformer.ConstrainedAxisDisplacementMode"/>
        ...
        /// </summary>
        /// <seealso cref="UnityEngine.XR.Interaction.Toolkit.Transformers.XRGeneralGrabTransformer.TwoHandedRotationMode"/>
        ...
        [Header("Bow Settings")]
        [SerializeField]
        [Tooltip("The pull interactable used for the bowstring. When it is being pulled, rotation is driven by the grip-to-notch direction.")]
        XRPullInteractable m_PullInteractable;

        public XRPullInteractable pullInteractable
        {
            get => m_PullInteractable;
            set
            {
                if (m_PullInteractable != null)
                    m_PullInteractable.PullStarted -= OnPullStarted;
                m_PullInteractable = value;
                if (m_PullInteractable != null && isActiveAndEnabled)
                    m_PullInteractable.PullStarted += OnPullStarted;
            }
        }
        ...
        [SerializeField]
        [Range(1f, 90f)]
        [Tooltip("Speed at which the bow rotates toward the grip-to-notch direction while pulling.")]
        float m_RotationSmoothing = 20f;
        ...
        void OnPullStarted()
        {
            if (m_GrabInteractable != null)
                m_LastBowRotation = m_GrabInteractable.transform.rotation;
        }
        ...
        public override void OnGrab(XRGrabInteractable grabInteractable)
        {
            ...
            m_LastBowRotation = grabInteractable.transform.rotation;
        ...
        void UpdateTarget(XRGrabInteractable grabInteractable, ref Pose targetPose, ref Vector3 localScale)
        {
            ...
            if (m_PullInteractable != null && m_PullInteractable.isPulling && grabInteractable.interactorsSelecting.Count > 0)
            {
                var bowGripPos = grabInteractable.attachTransform.position;
                var notchHandPos = m_PullInteractable.handPos;
                var fireDir = (bowGripPos - notchHandPos).normalized;

                if (fireDir != Vector3.zero)
                {
                    var upDir = -grabInteractable.interactorsSelecting[0].GetAttachTransform(grabInteractable).right;
                    var targetRot = Quaternion.LookRotation(fireDir, upDir);
                    targetPose.rotation = Quaternion.Slerp(m_LastBowRotation, targetRot, m_RotationSmoothing * Time.deltaTime);
                }
            }

            m_LastBowRotation = targetPose.rotation;
        }
    }
}
